...
<Tip>
  Public endpoints require [`NEXT_PUBLIC_PORTAL_URL`](/self-hosting/configuration) to be set and a note in your grimoire to carry the `#shareRoot` label. See [Sharing lore with players](/guides/sharing-lore) for the full setup.
</Tip>
