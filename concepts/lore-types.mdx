...
AllCodex structures your grimoire using **21 canonical lore entity types**. When the AI brain (AllKnower) extracts elements from your raw text or dumps, it assigns each note a specific `#loreType` label and maps it to a database template.
...
We can group the 21 lore types into four logical categories:
...
* **`creature`** (`_template_creature`): In-world beasts, monsters, or plants (less mechanical, more narrative than a statblock). Promotes: `creatureType`, `habitat`, `diet`, `dangerLevel`, `abilities`.
