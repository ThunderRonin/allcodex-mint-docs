...
* **Caching and deduplication**: To keep token usage low and performance high, suggestions are cached by content hash. Repeating a suggestion on an unmodified note is instant and costs zero tokens. In-flight suggestions are also deduplicated to prevent parallel LLM calls.
