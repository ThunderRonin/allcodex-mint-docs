...
The **Brain Dump** is the central ingestion pipeline of AllCodex. Dump raw thoughts or TTRPG session recaps, and the AI builds your wiki by creating notes, linking templates, and writing attributes.
...
| **Inbox** | The AI places the entire dump as a single raw note into your AllCodex Core "Inbox" folder without running LLM extraction. | Dumps that do not require immediate API token use. |
...
1. **Live stream**: The dashboard renders the AI's step-by-step reasoning and a stream of extracted entities (with titles and types) in real time.
